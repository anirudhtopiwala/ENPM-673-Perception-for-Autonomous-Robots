...
function [gmObj]= fitdata(xtot,k,numberiter)

% This function is used to return the multivariate gaussian object 
%% Line Space Data
% dataset=0;
...
% cd .. ; cd Part2;
% xtot= redbuoy;
% xtot=(yellowbuoy(:,1)+yellowbuoy(:,2))/2;
xtot=sort(xtot);
...
% plot(xtot,y2,'r')
% plot(xtot,y3,'r')
% title('Comparison of 3D Gaussians')
% xtot= [x1, x2, x3];
...
%     plot(xtot,ynew,'r','LineWidth',4);
%     plot(xtot,ynewmatlab,'g','LineWidth',2);
% %     legend('Hisotgram for 1D Input','My EM Output','Matlab EM Output');
% elseif (dataset==0)
%    for j=1:k
